...
class LocalEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.al_encoder = ALEncoder(
            # node_dim=4, #2 for w/o uncertainty, 4 for w/ uncertainty
            # edge_dim=4, #2 for w/o uncertainty, 4 for w/ uncertainty
            node_dim=6, # for advanced uncertainty
            edge_dim=6, # for advanced uncertainty
            embed_dim=embed_dim,
            num_heads=num_heads,
            dropout=dropout
        )
...
class ALEncoder(MessagePassing):
    ...
    def message(self,
            ...
            if rotate_mat is None:
                ...
                sigma_x, sigma_y = x_j[:, 2], x_j[:, 3]
                corr = x_j[:, 4]  
                cov_matrix = torch.zeros(x_j.size(0), 2, 2)
                cov_matrix[:, 0, 0] = sigma_x ** 2
                cov_matrix[:, 1, 1] = sigma_y ** 2
                cov_matrix[:, 0, 1] = cov_matrix[:, 1, 0] = corr * sigma_x * sigma_y
                cov_matrix = cov_matrix.to(x_j.device)
                cov_matrix_flat = cov_matrix.reshape(x_j.size(0), -1)

                x_j = self.lane_embed([
                    torch.cat([torch.bmm(x_j[:, :2].unsqueeze(-2), rotate_mat).squeeze(-2), cov_matrix_flat], dim=-1),
                    torch.cat([torch.bmm(edge_attr[:, :2].unsqueeze(-2), rotate_mat).squeeze(-2), cov_matrix_flat], dim=-1)
                ])
