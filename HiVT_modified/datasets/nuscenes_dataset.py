...
def get_lane_features(node_inds,
    ...
    node_positions = torch.cat([node_positions, torch.zeros(node_positions.shape[0], 1)], dim=-1)

    if centerline_used:
        centerlines = predicted_map['centerline']
        centerline_betas = predicted_map['centerline_betas']
        centerline_corrs = predicted_map['centerline_corrs']
    else:
        ...
        if boundary.shape[0] == 0:
            ...
            centerline_corrs = predicted_map['divider_corrs'][divider_indices]
        else:
            centerlines = boundary
            centerline_betas = predicted_map['boundary_betas'][boundary_indices]
            centerline_corrs = predicted_map['boundary_corrs'][boundary_indices]
    ...
    for i, centerline in enumerate(centerlines):
        ...
        lane_position_corrs.append(torch.tensor(centerline_corrs[i])) 
        count = len(lane_centerline) - 1

    lane_positions = torch.cat(lane_positions, dim=0)
    lane_position_betas = torch.cat(lane_position_betas, dim=0)
    lane_position_corrs = torch.cat(lane_position_corrs, dim=0)
    lane_positions = torch.cat((lane_positions, lane_position_betas, lane_position_corrs), dim=-1)
