...
class MapDetectorHead(nn.Module):
    ...
    def forward_train(self, bev_features, img_metas, gts):
        ...
        for i in range(len(final_inter_references)):
            ...
            corrs.append(final_inter_references[i][:, :, :, 4:5])
    ...
    def forward_test(self, bev_features, img_metas):
        ...
        for i, (queries) in enumerate(inter_queries):
            ...
            reg_points = reg_points.contiguous().view(bs, -1, 2*self.num_points) # (bs, num_q, 2*num_points)
            beta = beta.contiguous().view(bs, -1, 2*self.num_points) # (bs, num_q, 2*num_points)
            corr = corr.contiguous().view(bs, -1, self.num_points)
    ...
    def loss_single(self,
        ...
        corrs = torch.cat(preds['corrs'], dim=0)       # 400, 20
        ...
        loss_reg = self.loss_reg(torch.cat([pred_lines, betas, corrs], dim=-1), gt_lines, line_weights, avg_factor=num_total_pos)
    ...
    def post_process(self, preds_dict, tokens, thr=0.0):
        ...
        for i in range(bs):
            ...
            tmp_corrs = tmp_corrs.view(num_preds, num_points2//2, 1)
            ...
            if len(tmp_scores) == 0:
                ...
                single_result = {
                    ...
                    'corrs': tmp_corrs.detach().cpu().numpy(),
