...
def pts_nll_loss_multivariategaussian(pred, target):
    ...
    pred_reshape = pred.reshape(sample_size, 5)                  # [x_mean, y_mean, beta_x, beta_y, corr] 4000, 5
    pts_pred = pred_reshape[:, :2]                               # [x_mean, y_mean] 4000, 2
    pts_betas = pred_reshape[:, 2:4]                             # [beta_x, beta_y] 4000, 2
    corr = torch.clamp(pred_reshape[:, 4], -1 + 1e-5, 1 - 1e-5)  # [corr] 4000, 1

    # corr = torch.zeros_like(pred_reshape[:, 4])  # zeros --> corr 
...
class PtsNLLLoss(nn.Module):
    ...
    def forward(self,
        ...
        # breakpoint()
        loss_bbox = self.loss_weight * pts_nll_loss_multivariategaussian(pred, target, weight, reduction=reduction, avg_factor=avg_factor)
