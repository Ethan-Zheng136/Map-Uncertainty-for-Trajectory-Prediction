...
def scale_stream(data):
    ...
    new_corrs = corrs[indices]

    return new_pts, new_betas, new_corrs
...
def main(args):
    ...
    for i in tqdm(range(len(map_data)), desc="Merging Map Estimation"):
        ...
        if args.map_model == "MapTR":
            ...
            predict_divider_corrs = map_data[i]['pts_bbox']['corrs_3d'][divider_index].numpy()
            predict_ped_crossing_corrs = map_data[i]['pts_bbox']['corrs_3d'][ped_crossing_index].numpy()
            predict_boundary_corrs = map_data[i]['pts_bbox']['corrs_3d'][boundary_index].numpy()
            ...
            predict_divider_corrs = map_data[i]['corrs'][divider_index]
            predict_ped_crossing_corrs = map_data[i]['corrs'][ped_crossing_index]
            predict_boundary_corrs = map_data[i]['corrs'][boundary_index]
        ...
        if args.centerline:
            ...
            predict_centerlines_corrs = map_data[i]['pts_bbox']['corrs_3d'][centerline_index].numpy()
            ...
            predicted_map = {
                ...
                'divider_corrs': predict_divider_corrs,
                'ped_crossing_corrs': predict_ped_crossing_corrs,
                'boundary_corrs': predict_boundary_corrs,
                'centerline_corrs': predict_centerlines_corrs,
